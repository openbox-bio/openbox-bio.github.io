...
- **`has value type <ValueType>`**  
  ...
  ```
See Appendix A for accepted value types.
...
- **`has format <FormatType>`**  
  ...
  ```
See Appendix B for formats that can be specified. Note: the current formats apply only to the date-time data type. In future,
...
["HH:mm:ss" , "HH:mm" , "hh:mm:ss AM/PM" , "hh:mm AM/PM" , "HHmmss" , "YYYY-MM-DD HH:mm:ss" , 
